...
function openMenu() {
  ...
  // STEP 1 → gold flash start
  document.body.classList.add("menu-opening");

  // STEP 2 → after delay → final white state
  setTimeout(() => {
  ...
  if (ctaInner) {
    gsap.fromTo(
      ...
      { opacity: 0, y: 40 },
      {
        ...
        ease: "power3.out",
        scrollTrigger: {
          trigger: ctaInner.parentElement,
          start: "top 75%",
          toggleActions: "play none none none",
        },
      },
...
function initTilt() {
  document
    .querySelectorAll(".h-card, .p-card, .svc-card, .why-card, .pillar-card")
    .forEach((card) => {
...
function initContactForm() {
  ...
  form.addEventListener("submit", async (e) => {
    ...
    let ok = true;
    form
      .querySelectorAll("[required].form-inp,[required].form-ta")
      .forEach((el) => {
    ...
    if (cb && !cb.checked) {
      ok = false;
      cb.style.outline = "2px solid #C83C3C";
    }
    if (!ok) {
    ...
    setTimeout(() => {
      ...
      if (s) {
        ...
        gsap.from(s, { opacity: 0, y: 10, duration: 0.5 });
      }
    }, 1800);
  });
}
